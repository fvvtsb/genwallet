...
int main(int argc, char * * argv) {
	...
	try {
		...
		cl_int buildStatus = clBuildProgram(clProgram, vDevices.size(), vDevices.data(), strBuildOptions.c_str(), NULL, NULL);

		size_t sizeLog;
		clGetProgramBuildInfo(clProgram, vDevices[0], CL_PROGRAM_BUILD_LOG, 0, NULL, &sizeLog);
		if (sizeLog > 2) { // 1 byte means just \0 or empty
			char * const szLog = new char[sizeLog];
			clGetProgramBuildInfo(clProgram, vDevices[0], CL_PROGRAM_BUILD_LOG, sizeLog, szLog, NULL);
			std::cout << std::endl << "build log:" << std::endl << szLog << std::endl;
			delete[] szLog;
		}

		if (printResult(buildStatus)) {
			return 1;
		}
