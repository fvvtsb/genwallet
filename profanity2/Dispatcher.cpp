...
cl_kernel Dispatcher::Device::createKernel(cl_program & clProgram, const std::string s) {
	cl_int err;
	cl_kernel ret  = clCreateKernel(clProgram, s.c_str(), &err);
	return ret == NULL ? throw std::runtime_error("failed to create kernel \"" + s + "\", error code: " + std::to_string(err)) : ret;
}
...
void Dispatcher::handleResult(Device & d) {
	for (auto i = PROFANITY_MAX_SCORE; i >= m_clScoreMax && i > 0; --i) {
		...
		if (r.found > 0 && i >= d.m_clScoreMax) {
			...
			// Xóa cờ found để round sau có thể tìm tiếp ví cùng điểm số
			r.found = 0;
